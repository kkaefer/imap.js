 ...
 *   The line when there is one, undefined otherwise.
 */
StreamingBuffer.prototype.getBytes = function(bytes, callback, complete, data) {
  // Don't continue processing requests for 0 bytes.
  if (!bytes) {
    complete(data);
    return true;
  }
...
StreamingBuffer.prototype.getLine = function(callback, context) {
  var line;
  if ((line = this.nextLine()) !== undefined) {
    process.nextTick(function() {
      callback.call(context, line);
    });
  }
  else {
    this.on('data', function listener() {
      if ((line = this.nextLine()) !== undefined) {
        this.removeListener('data', listener);
        callback.call(context, line);
      }
