...
exports.utf8to7 = function(str) {
  ...
  return str.replace(/&/g, '&-').replace(/[^\x20-\x7e]+/g, function(s) {
    // & is represented by an empty sequence &-
    if (s === '&') return '&-';
    ...
    for (var i = 0, bi = 0; i < s.length; i++) {
      ...
      b[bi++] = c & 0xFF; // Lower 8 bits. Cut off the upper 8 bits so that they fit into 1 byte.
    }
